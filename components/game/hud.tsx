...
import { ELEMENT_META, JOB_TIERS, jobTierForLevel, jobTitle } from '@/lib/constants'
      ...
      <div className="pointer-events-auto flex flex-col gap-1.5">
        ...
        {state.settings.testMode && (
          <div className="flex flex-col items-end gap-1">
            <div className="panel-gilded flex items-center gap-1 px-2 py-1 text-[10px] text-gold-soft">
              <Sparkles className="size-3" /> 테스트 모드 ON
            </div>
            <button
              onClick={() => dispatch({ type: 'FORCE_BATTLE' })}
              className="panel-gilded px-2 py-1 text-[10px] text-gold-soft hover:brightness-125"
            >
              ⚔ 전투 테스트 (가까운 마수)
            </button>
          </div>
