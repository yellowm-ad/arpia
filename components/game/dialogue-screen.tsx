...
import { JOB_TIERS, jobTierForLevel, jobTitle } from '@/lib/constants'
...
export function DialogueScreen() {
  ...
  const eligibleTitle = jobTitle(state.player.element, eligible.order)
  const currentTitle = jobTitle(state.player.element, currentTier.order)
            ...
            <div className="mt-3 flex flex-wrap items-center justify-end gap-2">
              ...
              {lastLine && isJobTrainer &&
                (canJobChange ? (
                  ...
                  >
                    {eligibleTitle}(으)로 전직
                  </Button>
                ) : (
                  <span className="self-center text-[11px] text-muted-foreground">
                    현재 {currentTitle} · 다음 전직 Lv.
