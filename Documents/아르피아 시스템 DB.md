...
## 13. 구현 상태 (arpia 코드베이스, 1차)

`잡다한거/arpia/` Next.js 프로토타입에 반영 완료. `npx tsc --noEmit` 및 `next build` 통과.

| 영역 | 상태 |
|---|---|
| 3속성(불꽃/얼음/대지) + 상성 1.5×/0.67× + 종족 일치 1.3× | ✅ `lib/constants.ts` |
| 전직 5단계(견습/초보/숙련/마도사/대마도사, Lv 1·10·20·30·40) + 레벨업 자동 스킬 습득 | ✅ |
| 8스탯 + 성장식 + EXP 테이블(메이플 환산) | ✅ |
| **ATB 대기 게이지** 전투 (충전·행동·후딜·감속 반영) | ✅ `lib/battle-engine.ts` |
| **상태이상 9종** (지속피해·행동제약·명중·ATB·스탯 훅) + 감염 전염 | ✅ `lib/status-effects.ts` |
| **펫**: 도감 8종, 호감도 4구간(스탯·치명타·지원공격), 스킬 훈련(조련사 화면) | ✅ `lib/pets.ts`, `components/game/tamer-screen.tsx` |
| 스킬 62종(속성 27 + 무속성 6 + 펫 23) — 위력/MP/상태이상/버프 | ✅ `lib/mock-data.ts` |
| 몬스터 24종 + family/trait + 드롭 테이블 | ✅ |
| 아이템 43종(완드 15·방어구 8·장신구 6·소모품 10·먹이 4) | ✅ |
| 맵: 기존 7구역 재명명 + **아즈카의 폐허** 신규(Lv 10~32) | ✅ |
| 아트: 3속성/무 + 펫 8종 코드 생성 SVG | ✅ (플레이스홀더) |
| 파티 다중 펫(최대 2 동시 출전) | ⏳ 데이터 모델만. 현재 활성 1마리 |
| 무기 속성 시너지(+8%) 전투 반영 | ⏳ 장비 스탯만 반영, 시너지 계수 미적용 |
