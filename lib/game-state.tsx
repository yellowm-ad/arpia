...
export type Action =
  ...
  | { type: 'FORCE_BATTLE' } // 테스트 모드 전용: 가장 가까운 필드 마수와 즉시 전투
  ...
  switch (action.type) {
    ...
    case 'JOB_CHANGE': {
      ...
      return {
        ...
        toast: `${jobTitle(state.player.element, eligible.order)}(으)로 전직했습니다!`,
      }
    }
    ...
    case 'FORCE_BATTLE': {
      if (state.battle || state.fieldMonsters.length === 0) return state
      const px = state.position.x
      const py = state.position.y
      const nearest = [...state.fieldMonsters].sort(
        (a, b) =>
          Math.hypot(a.homeCell.x - px, a.homeCell.y - py) -
          Math.hypot(b.homeCell.x - px, b.homeCell.y - py),
      )[0]
      return startBattleFromField(state, nearest.uid)
    }
